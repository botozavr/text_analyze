...
def analyze_file(path: str, top: int = 10) -> DocumentStats:
    """Анализ файла
    path - путь к файлу
    ...
    return DocumentStats(path = path, word_count = len(dict), top_words = list(sorted_dict.items())[:top])
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="Пример использования: python -m wordfreq.py path/to/file.txt --top 20"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default = 10,
        help="Количество топ-слов для вывода (по умолчанию: 10)"
    )

    args = parser.parse_args()

    result = analyze_file(args.input_file, args.top)
    print(f"-" * 30)
    print('Анализ файла')
    print(f"-" * 30)
